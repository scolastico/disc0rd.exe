...
function saveConfig() {
  ...
  xmlHttpSettingsSend.onreadystatechange = function() {
    if (xmlHttpSettingsSend.readyState === 4 &&
      ...
      if (JSON.parse(xmlHttpSettingsSend.responseText).status === "ok") {
        ...
        return;
      } else if (JSON.parse(xmlHttpSettingsSend.responseText).status ===
                 "error") {
        if (JSON.parse(xmlHttpSettingsSend.responseText).error ===
            "no otp auth") {
          noOtpAuth();
          return;
        }
    ...
    if (xmlHttpSettingsSend.readyState === 4 &&
      ...
      if (JSON.parse(xmlHttpSettingsSend.responseText).status === "ok") {
        document.getElementById("config").value =
            beautifyJsonString(JSON.stringify(
                JSON.parse(xmlHttpSettingsSend.responseText).config));
...
function sendSettings() {
  ...
  xmlHttpSettingsSend.setRequestHeader("Content-type",
...
xmlHttpSettings.onreadystatechange = function() {
  if (xmlHttpSettings.readyState === 4 && xmlHttpSettings.status === 200) {
    if (JSON.parse(xmlHttpSettings.responseText).status === "ok") {
      document.getElementById("w2gDefaultPlayback").value =
          JSON.parse(xmlHttpSettings.responseText).settings.w2gDefaultPlayback;
    } else {
      window.location.replace("logout.html");
    }
  }
