function noOtpAuth() {
  ...
  Toast.fire({ type: "error", title: "OTP Not Valid!" });
}
...
function saveConfig() {
  ...
  xmlHttpSettingsSend.open(
    "POST",
    "../api/v1/admin/saveConfig/" + document.getElementById("otp").value,
    true
  );
  document.getElementById("otp").value = "";
  ...
  xmlHttpSettingsSend.onreadystatechange = function () {
    if (
      xmlHttpSettingsSend.readyState === 4 &&
      xmlHttpSettingsSend.status === 200
    ) {
      if (JSON.parse(xmlHttpSettingsSend.responseText).status === "ok") {
        ...
        JSON.parse(xmlHttpSettingsSend.responseText).status === "error"
      ) {
        if (
          JSON.parse(xmlHttpSettingsSend.responseText).error === "no otp auth"
        ) {
          noOtpAuth();
          return;
        }
...
function loadConfig() {
  ...
  xmlHttpSettingsSend.open(
    "POST",
    "../api/v1/admin/getConfig/" + document.getElementById("otp").value,
    true
  );
  document.getElementById("otp").value = "";
  xmlHttpSettingsSend.onreadystatechange = function () {
    if (
      ...
      if (JSON.parse(xmlHttpSettingsSend.responseText).status === "ok") {
        document.getElementById("config").value = beautifyJsonString(
          JSON.stringify(JSON.parse(xmlHttpSettingsSend.responseText).config)
        );
        ...
        Toast.fire({ type: "success", title: "Loaded!" });
        return;
      } else if (
        JSON.parse(xmlHttpSettingsSend.responseText).status === "error"
      ) {
...
function sendSettings() {
  ...
  xmlHttpSettingsSend.setRequestHeader(
    "Content-type",
