...
function fireSuccessToast() { fireToast("success", "Success!"); }
          ...
          if (xmlHttp1.readyState === 4 && xmlHttp1.status === 200) {
            ...
            if (info.status === "ok") {
              ...
              document.getElementById('main-container').innerHTML +=
                  getHtmlPermissionsCard('create', defaultValues,
                                         availablePermissions, '0', false);
            ...
            if (info.status === "ok") {
              ...
              for (const [permission, description] of Object.entries(
                ...
                permissions[permission] =
                    document.getElementById(id + '-permission-' + permission)
                        .checked;
              }
              let obj = {
                'isUser' :
                    document.getElementById(id + '-isUserPermission').checked,
                'id' : document.getElementById(id + '-id').value,
                'permissions' : permissions
              };
              const xmlHttp2 = new XMLHttpRequest();
              xmlHttp2.open("POST", "../api/v1/guild/permissions/set/" + id,
                            true);
              ...
              xmlHttp2.onreadystatechange = function() {
                try {
                  if (xmlHttp2.readyState === 4 && xmlHttp2.status === 200) {
                    ...
                    if (info.status === "ok") {
                      let classList =
                          document.getElementById(id + '-warning').classList;
                      ...
                      if (id === "create") {
                        ...
                        document.getElementById('main-container').innerHTML +=
                            getHtmlPermissionsCard(info.uuid, permissions,
                                                   availablePermissions, obj.id,
                                                   obj.isUser);
                      }
                    } else {
                      fireErrorSwal();
        ...
        .then((result) => {
          if (result.value) {
            if (id === "create") {
              ...
              const xmlHttp1 = new XMLHttpRequest();
              xmlHttp1.open("GET", "../api/v1/guild/permissions/delete/" + id,
                            true);
              xmlHttp1.onreadystatechange = function() {
                try {
                  if (xmlHttp1.readyState === 4 && xmlHttp1.status === 200) {
                ...
                if (xmlHttp2.readyState === 4 && xmlHttp2.status === 200) {
                  ...
                  if (info.status === "ok") {
                    ...
                    for (const [uuid, group] of Object.entries(groups)) {
                      cards += getHtmlPermissionsCard(uuid, group.permissions,
                                                      availablePermissions,
                                                      group.id, group.isUser);
                    }
                    document.getElementById('main-container').innerHTML +=
                        cards;
                  } else {
