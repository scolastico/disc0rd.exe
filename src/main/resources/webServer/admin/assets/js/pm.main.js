...
try {
    ...
    function create() {
        if (document.getElementById('create-card')) {
            fireToast('error', 'Please save the group first before you create another one.');
        } else {
                ...
                try {
                    if (xmlHttp1.readyState === 4 && xmlHttp1.status === 200) {
                        ...
                        if (info.status === "ok") {
                            ...
                            document.getElementById('main-container').innerHTML += getHtmlPermissionsCard('create', defaultValues, availablePermissions, '0', false);
                    ...
                    if (xmlHttp1.readyState === 4 && xmlHttp1.status === 200) {
                        ...
                        if (info.status === "ok") {
                            ...
                            for (const [permission, description] of Object.entries(availablePermissions)) {
                                permissions[permission] = document.getElementById(id + '-permission-' + permission).checked;
                            }
                            let obj = {
                            ...
                            xmlHttp2.open( "POST", "../api/v1/guild/permissions/set/" + id, true);
                            ...
                            xmlHttp2.onreadystatechange = function() {
                                try {
                                    if (xmlHttp2.readyState === 4 && xmlHttp2.status === 200) {
                                        ...
                                        if (info.status === "ok") {
                                            let classList = document.getElementById(id + '-warning').classList;
                                            if (!classList.contains('invisible')) classList.add('invisible');
                                            fireToast("success", "Saved!");
                                            if (id === "create") {
                                                document.getElementById('create-card').remove();
                                                document.getElementById('main-container').innerHTML += getHtmlPermissionsCard(info.uuid, permissions, availablePermissions, obj.id, obj.isUser);
                                            }
                                        } else {
                                            fireErrorSwal();
                                        }
        ...
        Swal.fire({
            ...
            if (result.value) {
                if (id === "create") {
                    ...
                    const xmlHttp1 = new XMLHttpRequest();
                    xmlHttp1.open( "GET", "../api/v1/guild/permissions/delete/" + id, true);
                    xmlHttp1.onreadystatechange = function() {
                        try {
                            if (xmlHttp1.readyState === 4 && xmlHttp1.status === 200) {
                                const info = JSON.parse(xmlHttp1.responseText);
                                ...
                                if (xmlHttp2.readyState === 4 && xmlHttp2.status === 200) {
                                    ...
                                    if (info.status === "ok") {
                                        ...
                                        for (const [uuid, group] of Object.entries(groups)) {
                                            cards += getHtmlPermissionsCard(uuid, group.permissions, availablePermissions, group.id, group.isUser);
                                        }
                                        document.getElementById('main-container').innerHTML += cards;
                                    } else {
                                        fireErrorSwal();
                                    }
                                }
