...
public class MusicPlayer {
    ...
    public MusicPlayer(VoiceChannel channel, TextChannel textChannel) {
        ...
        player = MusicPlayerRegister.getInstance().getPlayerManager().createPlayer();
    ...
    public void playInstantly(AudioTrack track) {
    ...
    public void addToQueue(String trackUrl) {
        try {
            ...
            if (matcher.find()) {
                ...
                builder.setDescription("but i have trouble while converting this song/playlist to YouTube.");
            ...
            playerManager.loadItemOrdered(this, trackUrl, new AudioLoadResultHandler() {
                ...
                public void trackLoaded(AudioTrack track) {
                    ...
                    builder.setDescription("**" + track.getInfo().title + "** [" + track.getInfo().author + "]");
                    textChannel.sendMessage(builder.build()).queue();
                    addToQueue(track);
                }
                ...
                public void playlistLoaded(AudioPlaylist playlist) {
                    ...
                    if (playlist.isSearchResult()) {
                        builder.setTitle("Added Song to Queue");
                        builder.setDescription("**" + firstTrack.getInfo().title + "** [" + firstTrack.getInfo().author + "]");
                    } else {
                        builder.setTitle("Added Playlist to Queue");
                        builder.setDescription("**" + playlist.getName() + "** (" + playlist.getTracks().size() + " Tracks)");
                ...
                public void loadFailed(FriendlyException exception) {
                    ...
                    builder.setColor(Color.RED);
                    builder.setTitle("Sorry,");
                    builder.setDescription("but there was an exception while trying to play this song: `" + exception.getMessage() + "`");
                    textChannel.sendMessage(builder.build()).queue();
                }
            });
        } catch (Exception e) {
        ...
        if (!guild.getAudioManager().isConnected() || member.getVoiceState() == null) {
            ...
            if (!member.getVoiceState().isDeafened()) {
                try {
                    ...
                    builder.setDescription("please dont un- deafen me. This is required to save resources.\nOr do you want to pay my server bill?");
                    textChannel.sendMessage(builder.build()).queue();
