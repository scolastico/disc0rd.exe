...
public class Pr0grammAPI {
    ...
    public static class Pr0grammPost {
        ...
        private Pr0grammPost(long id, boolean promoted, long userId, int up, int down, long created, String image, String thumbnail, String full, int width, int height, boolean audio, String source, int flags, String user, int mark, int gift) {
            this.id = id;
    ...
    public static class Pr0grammUser {
        ...
        public long getCommentsDeleted() {
    ...
    public static class Pr0grammGetItemsRequestGenerator {
        ...
        private String generate() {
            String ret = "";
            if (older != null) ret += "&older=" + older.toString();
            if (newer != null) ret += "&newer=" + newer.toString();
            if (flagCalculator != null) ret += "&flags=" + flagCalculator.calculate();
            if (promoted) ret += "&promoted=1"; else ret += "&promoted=0";
            if (user != null) ret += "&user=" + user;
            if (likes != null) ret += "&likes=" + likes;
            try {
                if (tags != null) ret += "&tags=" + URLEncoder.encode(tags, StandardCharsets.UTF_8.toString());
            } catch (UnsupportedEncodingException ignored) {}
            return "items/get?" + ret.substring(1);
        }

        public Pr0grammAPI getApi() {
            return api;
        }
        ...
        public Pr0grammFlagCalculator getFlagCalculator() {
