...
public class Pr0grammManager {
    ...
    private HashMap<Long, ServerSettings.Pr0grammServerConfig> pr0grammServerConfigs;
    ...
    public void checkForNewPosts() {
        try {
            ...
            generator.setFlagCalculator(calculator);
            Pr0grammAPI.Pr0grammPost[] postsNSFL = pr0grammAPI.getPr0grammPosts(generator);
            ...
            for (Integer flag:posts.keySet()) {
                for (Pr0grammAPI.Pr0grammPost post:posts.get(flag)) {
                    ...
                    for (Long guildId:pr0grammServerConfigs.keySet()) {
                        ...
                        ServerSettings.Pr0grammServerConfig config = pr0grammServerConfigs.get(guildId);
                        for (ServerSettings.Pr0grammServerConfig.Pr0grammSubscription subscription:config.getSubscriptions()) {
                            if (subscription.getUsername().equals(post.getUser())) {
                                if ((flag == 1 && subscription.isSfw()) || (flag == 2 && subscription.isNsfw()) || (flag == 3 && subscription.isNsfl())) {
                                    TextChannel channel = guild.getTextChannelById(subscription.getChannel());
                                    if (channel == null) continue;
                                    EmbedBuilder builder = new EmbedBuilder();
                                    builder.setTitle("Pr0gramm.com");
                                    builder.setDescription("New upload found! <https://pr0gramm.com/new/" + post.getId() + ">");
    ...
    public void reloadConfigDataSore() {
        ...
        HashMap<Long, ServerSettings> serverSettingsHashMap = handler.getAllServerSettingsWithId();
        for (Long id:serverSettingsHashMap.keySet()) {
            if (serverSettingsHashMap.get(id).getPr0grammServerConfig().getLinkedAccount() != null) {
                pr0grammServerConfigs.put(id, serverSettingsHashMap.get(id).getPr0grammServerConfig());
            }
        }
    }

    public boolean authGuild(long id, String username, long memberId) {
        if (isAuthorizedGuild(id)) return false;
        MysqlHandler handler = Disc0rd.getMysql();
        ServerSettings settings = handler.getServerSettings(id);
        ServerSettings.Pr0grammServerConfig config = settings.getPr0grammServerConfig();
    ...
    public ArrayList<ServerSettings.Pr0grammServerConfig.Pr0grammSubscription> getSubsFromGuild(long id) {
        if (!isAuthorizedGuild(id)) return null;
    ...
    public boolean subscribeUser(long guildId, long channelId, String username) {
        if (!isAuthorizedGuild(guildId)) return false;
        MysqlHandler handler = Disc0rd.getMysql();
        ServerSettings.Pr0grammServerConfig serverConfig = pr0grammServerConfigs.get(guildId);
        for (ServerSettings.Pr0grammServerConfig.Pr0grammSubscription subscription:serverConfig.getSubscriptions()) {
            if (subscription.getChannel() == channelId && subscription.getUsername().equals(username)) return false;
        }
        serverConfig.addSubscription(new ServerSettings.Pr0grammServerConfig.Pr0grammSubscription(username, true, false, false, channelId));
    ...
    public boolean unSubscribeUser(long guildId, long channelId, String username) {
        if (!isAuthorizedGuild(guildId)) return false;
        MysqlHandler handler = Disc0rd.getMysql();
        ServerSettings.Pr0grammServerConfig serverConfig = pr0grammServerConfigs.get(guildId);
        ServerSettings.Pr0grammServerConfig.Pr0grammSubscription toDelete = null;
        for (ServerSettings.Pr0grammServerConfig.Pr0grammSubscription subscription:serverConfig.getSubscriptions()) {
            if (subscription.getChannel() == channelId && subscription.getUsername().equals(username)) {
    ...
    public boolean setFollowedChannels(long guildId, long channelId, String username, boolean sfw, boolean nsfw, boolean nsfl) {
        if (!isAuthorizedGuild(guildId)) return false;
        MysqlHandler handler = Disc0rd.getMysql();
        ServerSettings.Pr0grammServerConfig serverConfig = pr0grammServerConfigs.get(guildId);
        ServerSettings.Pr0grammServerConfig.Pr0grammSubscription toDelete = null;
        ...
        for (ServerSettings.Pr0grammServerConfig.Pr0grammSubscription subscription:serverConfig.getSubscriptions()) {
            if (subscription.getChannel() == channelId && subscription.getUsername().equals(username)) {
                toDelete = subscription;
                toAdd = new ServerSettings.Pr0grammServerConfig.Pr0grammSubscription(username, sfw, nsfw, nsfl, channelId);
            }
        }
        if (toDelete != null) {
            serverConfig.removeSubscription(toDelete);
    ...
    public void checkAllConnectedServersAndClear() {
        ...
        builder.setDescription("but the linked Pr0gramm account is not longer valid! There could be several reasons! E.g. the connected account has been banned or the account owner is no longer guild owner!");
        builder.setColor(Color.red);
        for (long key:pr0grammServerConfigs.keySet()) {
            ServerSettings.Pr0grammServerConfig config = pr0grammServerConfigs.get(key);
            ...
            if (guild != null) {
                ...
                if (member != null) {
                    if (Tools.getInstance().isOwner(guild, member.getUser())) {
                        try {
                            Pr0grammAPI.Pr0grammUser user = pr0grammAPI.getPr0grammUser(config.getLinkedAccount());
                            if (user.getBanned() == 0) {
                                continue;
                            }
