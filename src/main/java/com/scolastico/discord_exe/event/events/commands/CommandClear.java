...
public class CommandClear implements EventHandler, CommandHandler {
      ...
      if (args.length == 0) {
        event.getChannel()
            .sendMessage(
                ...
                "> Sorry, but i cant find this command. Check your arguments or try `disc0rd/help clear`.")
            .queue();
      } else if (args.length == 1) {
        try {
          if (PermissionsManager.getInstance().checkPermission(
                  event.getGuild(), member, "clear")) {
            int number = 1;
            try {
              ...
              event.getChannel()
                  .sendMessage(
                      "<:" + emoteNo.getName() + ":" + emoteNo.getId() +
                      "> Sorry, but please enter only numeric values as argument!")
                  .queue();
              return true;
            }
            if (number > 100 || number < 1) {
              event.getChannel()
                  .sendMessage(
                      "<:" + emoteNo.getName() + ":" + emoteNo.getId() +
                      "> Sorry, but you can only delete 1-100 messages at once!")
                  .queue();
            } else {
              MessageHistory messageHistory =
                  event.getChannel()
                      .getHistoryBefore(event.getMessageIdLong(), number)
                      .complete();
          ...
          event.getChannel()
              .sendMessage("<:" + emoteNo.getName() + ":" + emoteNo.getId() +
  ...
  public HashMap<String, String> getHelpSiteDetails() {
    ...
    helpSite.put("clear <number>",
                 "Delete a number of messages at once (1-100)");
    return helpSite;
  }
  ...
  public void registerEvents(EventRegister eventRegister) {
    ...
    PermissionsManager.getInstance().registerPermission(
        "clear", "Allow a user to use the clear command.", false);
  }
}
