...
public class CommandClearQueue implements EventHandler, CommandHandler {
    ...
    if (cmd.equalsIgnoreCase("clearQueue")) {
      ...
      if (args.length == 0) {
        if (PermissionsManager.getInstance().checkPermission(
                event.getGuild(), member, "clear-queue")) {
        ...
        event.getChannel()
            .sendMessage(
                "<:" + emoteNo.getName() + ":" + emoteNo.getId() +
                "> Sorry, but i cant find this command. Check your arguments or try `disc0rd/help clearQueue`.")
            .queue();
      }
      return true;
    }
  ...
  public void registerEvents(EventRegister eventRegister) {
    ...
    PermissionsManager.getInstance().registerPermission(
        "clear-queue",
        "Allow a user to use the clearQueue command from the music player.",
        true);
  }
}
