...
public class OnChatLeaderboard implements EventHandler, MessageReceivedHandler {
  ...
  public void registerEvents(EventRegister eventRegister) {
    ...
    PermissionsManager.getInstance().registerPermission(
        "leaderboard-collect-text",
        "Allow a user to collect points on the leaderboard via text chat.",
        true);
  }
  ...
  public void handleMessageReceived(MessageReceivedEvent messageReceivedEvent) {
    if (messageReceivedEvent.getChannel().getType() == ChannelType.TEXT) {
      ...
      Member member =
          messageReceivedEvent.getGuild().retrieveMember(user).complete();
      if (member == null)
        return;
      if (!user.isBot())
        if (PermissionsManager.getInstance().checkPermission(
                messageReceivedEvent.getGuild(), member,
                "leaderboard-collect-text")) {
          clearTimeOut();
          if (!timeOut.containsKey(user.getIdLong())) {
            timeOut.put(user.getIdLong(),
                        Tools.getInstance().getUnixTimeStamp() + 60);
