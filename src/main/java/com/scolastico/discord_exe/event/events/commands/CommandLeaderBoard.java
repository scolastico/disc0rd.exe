...
public class CommandLeaderBoard implements EventHandler, CommandHandler {
    ...
    if (cmd.equalsIgnoreCase("leaderboard")) {
      ...
      if (args.length == 0) {
        if (PermissionsManager.getInstance().checkPermission(
                event.getGuild(), member, "leaderboard")) {
          builder.setColor(Color.green);
          builder.setTitle("You can access the leader board from here:");
          builder.setDescription(
              Disc0rd.getConfig().getWebServer().getDomain() + "leaderboard/#" +
        ...
        if (args[0].equalsIgnoreCase("enable")) {
          ...
          if (Tools.getInstance().isOwner(event.getGuild(),
            ...
            for (Long timestamp : confirmHashMap.keySet()) {
              ...
              if (data.getId() == event.getGuild().getIdLong() &&
                  data.getCmd().equals("reset")) {
                ...
                builder.setTitle("Success,");
                builder.setDescription(
                    "i resetted the leaderboard database for you!");
            ...
            confirmHashMap.put(
                ...
                new ConfirmData("reset", event.getGuild().getIdLong()));
            builder.setTitle("WARNING!");
            builder.setDescription(
                "You are about to reset the entire leaderboard! Please enter `disc0rd/leaderboard reset` in the next 60 seconds again to confirm!");
          } else {
            builder.setDescription(
                "but only the guild owner has the permission to use this command!");
          }
          ...
          if (PermissionsManager.getInstance().checkPermission(
                  event.getGuild(), member, "rank-self")) {
        ...
        if (args[0].equalsIgnoreCase("banner")) {
          if (PermissionsManager.getInstance().checkPermission(
                  event.getGuild(), member, "rank-other")) {
            try {
              ...
              if (matcher.find()) {
                ...
                if (member != null) {
                  if (!member.getUser().isBot()) {
                    ...
                    builder.setDescription(
                        "but this user isn't a real user. I only take care of real users!");
                  }
                } else {
          ...
          if (Tools.getInstance().isOwner(event.getGuild(),
                ...
                if (args[1].equalsIgnoreCase("<@!" + id + ">")) {
                  if (!member.getUser().isBot()) {
                    ...
                    for (Long timestamp : confirmHashMap.keySet()) {
                      ConfirmData data = confirmHashMap.get(timestamp);
                      if (data.getId() == event.getGuild().getIdLong() &&
                          data.getCmd().equals("reset-" + member.getId())) {
                        confirmHashMap.remove(timestamp, data);
                        builder.setColor(Color.green);
                        builder.setTitle("Success,");
                        builder.setDescription("i deleted the user `" +
                    ...
                    confirmHashMap.put(
                        (System.currentTimeMillis() / 1000L) + 60,
                        new ConfirmData("reset-" + member.getId(),
                                        event.getGuild().getIdLong()));
                    builder.setTitle("WARNING!");
                    builder.setDescription(
                        "You are about to reset the rank from `" +
                        member.getUser().getAsTag() +
                        "`! Please enter `disc0rd/leaderboard reset " +
                        args[1] + "` in the next 60 seconds again to confirm!");
                  } else
                    builder.setDescription(
                        "but this user isn't a real user. I only take care of real users!");
                }
            ...
            builder.setDescription(
                "but only the owner has the permission to use this command!");
          }
          ...
          if (Tools.getInstance().isOwner(event.getGuild(),
            ...
            if (args[1].equalsIgnoreCase("mee6")) {
              ...
              for (Long timestamp : confirmHashMap.keySet()) {
                ConfirmData data = confirmHashMap.get(timestamp);
                if (data.getId() == event.getGuild().getIdLong() &&
                    data.getCmd().equals("import-mee6")) {
                  confirmHashMap.remove(timestamp, data);
                  ServerSettings settings =
                      Disc0rd.getMysql().getServerSettings(
                          event.getGuild().getIdLong());
                  settings.setLeaderboard(new ServerSettings.Leaderboard());
                  ...
                  builder.setTitle("Success,");
                  builder.setDescription(
                      "i imported the leaderboard from mee6 for you!");
                  event.getChannel().sendMessage(builder.build()).queue();
                  return true;
                }
              }
              confirmHashMap.put(
                  (System.currentTimeMillis() / 1000L) + 60,
                  new ConfirmData("import-mee6", event.getGuild().getIdLong()));
              builder.setTitle("WARNING!");
              builder.setDescription(
                  "This command will reset the entire leaderboard! Please enter `disc0rd/leaderboard import mee6` in the next 60 seconds again to confirm!");
            } else {
              builder.setDescription(
                  "but this bot isn't supported right now! But you can ask the developers to implement this bot! Send us a mail to `" +
                  Disc0rd.getConfig().getEmail() + "`");
            }
          } else {
            builder.setDescription(
                "but only the owner has the permission to use this command!");
  ...
  private boolean sendBanner(MessageReceivedEvent event, Member member) {
    ...
    for (Long timestamp : confirmHashMap.keySet()) {
      ConfirmData data = confirmHashMap.get(timestamp);
      if (data.getId() == event.getGuild().getIdLong() &&
          data.getCmd().equals("banner-" + member.getId())) {
        builder.setDescription(
            "but since the rendering of the banner takes up a lot of CPU, I will only render this once per 15 seconds per user!");
        event.getChannel().sendMessage(builder.build()).queue();
        return true;
      }
    }
    confirmHashMap.put((System.currentTimeMillis() / 1000L) + 15,
                       new ConfirmData("banner-" + member.getId(),
                                       event.getGuild().getIdLong()));
    try {
      if (member.getUser().isBot())
        return true;
      String svg =
          Tools.getInstance().getLeaderboardBannerSVG(event.getGuild(), member);
      File file =
          new File(Disc0rd.getConfig().getTmpDir() + event.getGuild().getId() +
                   "-" + member.getId() + ".png");
      PNGTranscoder coder = new PNGTranscoder();
      StringReader reader = new StringReader(svg);
      TranscoderInput input = new TranscoderInput(reader);
      FileOutputStream outputStream = new FileOutputStream(
          Disc0rd.getConfig().getTmpDir() + event.getGuild().getId() + "-" +
          member.getId() + ".png");
      ...
      if (file.exists()) {
        ...
        if (!file.delete()) {
          ErrorHandler.getInstance().handle(new Exception(
              "Cant delete file at location '" + file.getPath() + "'!"));
        }
        return true;
      } else {
        ErrorHandler.getInstance().handle(new Exception(
            "Cant find rendered image after rendering in location '" +
            file.getPath() + "'"));
        builder.setDescription(
            "but i cant render the image. No worries i already reported the error to the developers!");
      }
    } catch (Exception e) {
      ErrorHandler.getInstance().handle(e);
      builder.setDescription(
  ...
  public HashMap<String, String> getHelpSite(HashMap<String, String> helpSite) {
    helpSite.put(
        "leaderboard",
        "Request the leader board url or change settings from the leader board.");
  ...
  public HashMap<String, String> getHelpSiteDetails() {
    ...
    helpSite.put("leaderboard banner <username>",
                 "Request the leader board banner from an other guild member.");
    helpSite.put(
        "leaderboard import <mee6>",
        "Import the leader board from an other bot. Like MEE6. Admin Command! Only one use per bot per day!");
    helpSite.put("leaderboard enable",
                 "Enable the leaderboard. Admin Command!");
    helpSite.put("leaderboard disable",
                 "Disable the leaderboard. Admin Command!");
    helpSite.put("leaderboard reset",
                 "Reset the complete leaderboard. Admin Command!");
    helpSite.put("leaderboard reset <user>",
                 "Reset an explicit user. Admin Command!");
    return helpSite;
  }
  ...
  public void registerEvents(EventRegister eventRegister) {
    ...
    PermissionsManager.getInstance().registerPermission(
        "leaderboard", "Allow a user to query the leaderboard url.", true);
    PermissionsManager.getInstance().registerPermission(
        "rank-self", "Allow a user to query their own rank.", true);
    PermissionsManager.getInstance().registerPermission(
        "rank-other", "Allow a user to query the rank of another user.", true);
  }
