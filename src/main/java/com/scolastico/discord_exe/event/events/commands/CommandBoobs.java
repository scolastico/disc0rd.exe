...
public class CommandBoobs implements EventHandler, CommandHandler {
    ...
    public boolean respondToCommand(String cmd, String[] args, JDA jda, MessageReceivedEvent event, long senderId, long serverId, Member member) {
        if (cmd.equalsIgnoreCase("boobs")) {
            if (PermissionsManager.getInstance().checkPermission(event.getGuild(), member, "command-boobs")) {
                event.getMessage().delete().queue();
                Pr0grammAPI api = Pr0grammManager.getInstance().getPr0grammAPI();
                Pr0grammAPI.Pr0grammGetItemsRequestGenerator generator = api.generateGetItemsRequestGenerator();
                Random random = new Random();
                generator.setTags("! -video & -Scheide & -Möse & -Vagina & -Oc & s:1000 & -(x:random|x:" + random.nextInt(1000) + ") & boobs");
                ...
                try {
                    ...
                    event.getChannel().sendMessage("<:" + emoteNo.getName() + ":" + emoteNo.getId() + "> Sorry, there was an unknown exception while trying to get an image!").queue();
                }
            } else {
    ...
    public void registerEvents(EventRegister eventRegister) {
        ...
        PermissionsManager.getInstance().registerPermission("command-boobs", "Allows a user to request a random boobs picture. NSFW!", false);
    }
}
