    ...
    public boolean respondToCommand(String cmd, String[] args, JDA jda, MessageReceivedEvent event, long senderId, long serverId, Member member) {
        if (cmd.equalsIgnoreCase("pause")) {
            ...
            if (args.length == 0) {
                        ...
                        if (channel != null) {
                            ...
                            if (player != null) {
                                if (player.getChannel() == channel) {
                                    if (player.getStatus()) {
                                        player.pause();
                                        event.getMessage().addReaction(EmoteHandler.getInstance().getEmotePause()).queue();
                                    } else {
                                        player.play();
                ...
                event.getChannel().sendMessage("<:" + emoteNo.getName() + ":" + emoteNo.getId() + "> Sorry, but i cant find this command. Check your arguments or try `disc0rd/help pause`.").queue();
            }
            return true;
        }
        return false;
