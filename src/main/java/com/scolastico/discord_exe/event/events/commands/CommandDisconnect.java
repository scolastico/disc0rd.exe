...
public class CommandDisconnect implements CommandHandler, EventHandler {
    ...
    if (cmd.equalsIgnoreCase("disconnect")) {
      ...
      if (args.length == 0) {
        if (PermissionsManager.getInstance().checkPermission(
                event.getGuild(), member, "disconnect")) {
            ...
            if (channel != null) {
              ...
              if (player != null) {
                if (player.getChannel() == channel) {
                  MusicPlayerRegister.getInstance().killPlayer(
                      event.getGuild().getIdLong());
                  event.getMessage()
                ...
                event.getChannel()
                    .sendMessage("<:" + emoteNo.getName() + ":" +
                                 emoteNo.getId() +
                                 "> Sorry, but there is no player currently")
                    .queue();
              }
            }
          } else {
        ...
        event.getChannel()
            .sendMessage(
                "<:" + emoteNo.getName() + ":" + emoteNo.getId() +
                "> Sorry, but i cant find this command. Check your arguments or try `disc0rd/help disconnect`.")
            .queue();
      }
      return true;
    }
  ...
  public HashMap<String, String> getHelpSite(HashMap<String, String> helpSite) {
    helpSite.put(
        "disconnect",
        "Stop the music player and disconnect from the voice channel.");
    return helpSite;
  }
  ...
  public void registerEvents(EventRegister eventRegister) {
    ...
    PermissionsManager.getInstance().registerPermission(
        "disconnect",
        "Allow a user to use the disconnect command from the music player.",
        true);
  }
}
