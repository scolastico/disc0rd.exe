...
public class CommandPlay implements CommandHandler, EventHandler {
              ...
              if (player != null) {
                if (player.getChannel() == channel) {
                  if (!player.getStatus()) {
                    ...
                    event.getMessage()
                        .addReaction(EmoteHandler.getInstance().getEmoteNo())
                        .queue();
                  }
                } else {
                  event.getChannel()
                      .sendMessage(
                          ...
                          "> Sorry, but you need to be in the same voice channel as the bot.")
                      .queue();
                }
              } else {
                event.getChannel()
  ...
  public HashMap<String, String> getHelpSiteDetails() {
    ...
    ret.put("play <playlist url>",
            "Add an complete playlist to the queue and play it.");
    return ret;
  }
  ...
  public void registerEvents(EventRegister eventRegister) {
    ...
    PermissionsManager.getInstance().registerPermission(
        "play", "Allow a user to use the play command from the music player.",
        true);
  }
}
