...
public class Pr0grammOAuth implements WebHandler {
    ...
    private static void clearAuthKeys() {
        ...
        for (String key:authKeys.keySet()) {
            if (authKeys.get(key).getValidUntil() <= (System.currentTimeMillis() / 1000L)) {
                toDelete.add(key);
            }
        }
    ...
    public String onWebServer(HttpExchange httpExchange) {
        try {
            HashMap<String, String> getVariables = Tools.getInstance().getGetValuesFromHttpExchange(httpExchange);
            if (getVariables.containsKey("state") && getVariables.containsKey("authCode") && getVariables.containsKey("userId")) {
                ...
                if (authKeys.containsKey(state)) {
                    ...
                    if (token == null) return error_page;

                    String username = getUsername(token);
                    if (username == null) return error_page;

                    Pr0grammManager.getInstance().authGuild(authKeyInformation.guildId, username, authKeyInformation.memberId);
    ...
    private String getUsername(String authToken) {
        ...
        try {
            ...
            post.addHeader("Cookie", Pr0grammManager.getInstance().getPr0grammAPI().getToken());
    ...
    private String getAuthToken(String authCode, String userId) {
        ...
        try {
            ...
            Pr0grammConfigDataStore.OAuth oAuthConfig = Pr0grammManager.getInstance().getConfig().getOAuth();

            urlParameters.add(new BasicNameValuePair("authCode", authCode));
            urlParameters.add(new BasicNameValuePair("userId", userId));
            urlParameters.add(new BasicNameValuePair("clientId", oAuthConfig.getClientId()));
            urlParameters.add(new BasicNameValuePair("clientSecret", oAuthConfig.getSecret()));
    ...
    private static class AuthKeyInformation {
        ...
        public AuthKeyInformation(String key, long guildId, long memberId) {
            ...
            this.validUntil = (System.currentTimeMillis() / 1000L) + Pr0grammManager.getInstance().getConfig().getOAuth().getKeyValidUntil();
        }
