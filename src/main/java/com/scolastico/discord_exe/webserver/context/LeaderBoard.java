...
@WebHandler.WebHandlerRegistration(
    context = {"/api/v1/leaderboard/img/*", "/api/v1/leaderboard/get/*"})
public class LeaderBoard implements WebHandler {
  ...
  private String get(HttpExchange httpExchange) {
    try {
      String guildId = httpExchange.getRequestURI().getPath().replaceFirst(
          "/api/v1/leaderboard/get/", "");
      if (guildId.equals(""))
      ...
      if (guild != null) {
        ...
        ServerSettings settings =
            Disc0rd.getMysql().getServerSettings(guild.getIdLong());
          ...
          try {
            ...
            if (member != null)
              if (!member.getUser().isBot()) {
                ...
                int position =
                    Tools.getInstance().getLeaderboardPlace(guild, member);
                LeaderBoardData tmp = new LeaderBoardData(
                    ...
                    member.getId(), xp, level,
                    Tools.getInstance().getLeaderboardNextLevelXP(xp),
                    Tools.getInstance().getLeaderboardLevelXP(level), position);
  ...
  private static class LeaderBoardData {
    ...
    public LeaderBoardData(String name, String tag, String picture, String id,
                           long xp, int level, long nextLevelXP,
                           long lastLevelXP, int place) {
      this.name = name;
      this.tag = tag;
    ...
    public long getLastLevelXP() { return lastLevelXP; }
